...
def _render_aa_html_table(items: List[Dict], timeframe_label: str) -> str:
    """Renders a responsive HTML table for a single timeframe's AgeAgain stocks."""
    if not items:
        return ""
    ...
    for i, item in enumerate(items):
        ...
        sentiment = item.get("sentiment", "Neutral")
        if sentiment == "Bullish":
            sent_badge = '<span style="color: #4ade80; font-weight: bold;">▲ Absorption</span>'
        elif sentiment == "Bearish":
            sent_badge = '<span style="color: #f87171; font-weight: bold;">▼ Effort w/o Result</span>'
        else:
            sent_badge = f'<span style="color: #94a3b8;">{sentiment}</span>'

        vol_pct = f"{item.get('vol_delta_pct'):+0.1f}%" if item.get("vol_delta_pct") is not None else "--"
        spread_pct = f"{item.get('spread_delta_pct'):+0.1f}%" if item.get("spread_delta_pct") is not None else "--"
        cp_str = f"{item.get('t_cp'):.4f}" if item.get("t_cp") is not None else "--"
        ...
        <tr style="background-color: {bg_color}; border-bottom: 1px solid #334155;">
            ...
            <td style="padding: 7px 10px; color: #38bdf8; text-align: right;">{vol_pct}</td>
            <td style="padding: 7px 10px; color: #fbbf24; text-align: right;">{spread_pct}</td>
            <td style="padding: 7px 10px; color: #cbd5e1; text-align: right;">{cp_str}</td>
        </tr>"""
    ...
    <div style="margin-top: 14px;">
        ...
        <table style="width: 100%; border-collapse: collapse; font-size: 12px; text-align: left; background-color: #1e293b; border-radius: 6px; overflow: hidden;">
            <thead>
                <tr style="background-color: #0f172a; color: #94a3b8; font-size: 11px; text-transform: uppercase; letter-spacing: 0.5px;">
                    <th style="padding: 7px 10px;">Symbol</th>
                    <th style="padding: 7px 10px;">Scenario</th>
                    <th style="padding: 7px 10px; text-align: right;">Vol Δ%</th>
                    <th style="padding: 7px 10px; text-align: right;">Spread Δ%</th>
                    <th style="padding: 7px 10px; text-align: right;">Close Pos</th>
                </tr>
            </thead>
            <tbody>
                {rows_html}
...
def _format_age_again_email_html(aa_filters: Dict) -> str:
    """Generates the full HTML section with summary and detailed tables for AgeAgain."""
    daily_aa = aa_filters.get("daily", [])
    weekly_aa = aa_filters.get("weekly", [])
    monthly_aa = aa_filters.get("monthly", [])
    total_aa = len(daily_aa) + len(weekly_aa) + len(monthly_aa)

    if total_aa == 0:
        return ""

    all_aa = daily_aa + weekly_aa + monthly_aa
    bullish_count = sum(1 for v in all_aa if v.get("sentiment") == "Bullish")
    bearish_count = sum(1 for v in all_aa if v.get("sentiment") == "Bearish")

    daily_table = _render_aa_html_table(daily_aa, "Daily")
    weekly_table = _render_aa_html_table(weekly_aa, "Weekly")
    monthly_table = _render_aa_html_table(monthly_aa, "Monthly")

    return f"""
    <div style="background-color: #334155; padding: 20px; border-radius: 8px; margin: 30px 0; border-left: 4px solid #818cf8;">
        <h3 style="color: #a5b4fc; margin: 0 0 6px 0; font-size: 18px;">⚡ AgeAgain Structural Anomaly Insights</h3>
        <p style="color: #cbd5e1; margin: 0; font-size: 14px;">
            <strong>{total_aa}</strong> stocks detected across timeframes
            (Daily: {len(daily_aa)}, Weekly: {len(weekly_aa)}, Monthly: {len(monthly_aa)})
        </p>
        <p style="color: #94a3b8; margin: 6px 0 4px 0; font-size: 13px;">
            <span style="color: #4ade80; font-weight: bold;">▲ Absorption: {bullish_count}</span> &nbsp;|&nbsp;
            <span style="color: #f87171; font-weight: bold;">▼ Effort w/o Result: {bearish_count}</span>
        </p>
        {daily_table}
        {weekly_table}
        {monthly_table}
...
def _render_aa_text_table(items: List[Dict], timeframe_label: str) -> str:
    """Renders a formatted text table for a single timeframe's AgeAgain stocks."""
    if not items:
        return ""
    lines = [
        f"  📅 {timeframe_label} ({len(items)} stock{'s' if len(items) > 1 else ''}):",
        f"  {'SYMBOL':<12} {'SENTIMENT':<10} {'VOL Δ%':>10} {'SPREAD Δ%':>10} {'CLOSE POS':>10}",
        f"  {'-'*54}"
    ]
    for item in items:
        sym = item.get("symbol", "--")
        ...
        vol = f"{item.get('vol_delta_pct'):+0.1f}%" if item.get("vol_delta_pct") is not None else "--"
        spread = f"{item.get('spread_delta_pct'):+0.1f}%" if item.get("spread_delta_pct") is not None else "--"
        cp = f"{item.get('t_cp'):.4f}" if item.get("t_cp") is not None else "--"
        lines.append(f"  {sym:<12} {sent:<10} {vol:>10} {spread:>10} {cp:>10}")
    lines.append("")
    return "\n".join(lines)


def _format_age_again_email_text(aa_filters: Dict) -> str:
    """Generates the full plain-text report for AgeAgain."""
    daily_aa = aa_filters.get("daily", [])
    weekly_aa = aa_filters.get("weekly", [])
    monthly_aa = aa_filters.get("monthly", [])
    total_aa = len(daily_aa) + len(weekly_aa) + len(monthly_aa)
    ...
    lines = [
        f"\n⚡ AgeAgain Structural Anomaly Insights: {total_aa} stocks detected (Daily: {len(daily_aa)}, Weekly: {len(weekly_aa)}, Monthly: {len(monthly_aa)})",
        f"   Absorption: {bullish_count} | Effort w/o Result: {bearish_count}\n"
    ]
    for label, items in [("Daily", daily_aa), ("Weekly", weekly_aa), ("Monthly", monthly_aa)]:
        tbl = _render_aa_text_table(items, label)
        if tbl:
            lines.append(tbl)
...
def main():
    ...
    # Load dashboard data once (Single-Read DRY Pattern)
    data = {}
    try:
        ...
        logger.warning("FAILED_TO_LOAD_DATA_JSON", extra={"error": str(e)})

    # Extract Macro Intelligence from loaded data
    macro_html = ""
    macro_text = ""
    ...
    # Extract Volume Trap Filter details from loaded data
    vt_filters = data.get("volume_trap_filters", {})
    volume_trap_html = _format_volume_trap_email_html(vt_filters) if vt_filters else ""
    volume_trap_text = _format_volume_trap_email_text(vt_filters) if vt_filters else ""

    # Extract AgeAgain Filter details from loaded data
    aa_filters = data.get("age_again_filters", {})
    age_again_html = _format_age_again_email_html(aa_filters) if aa_filters else ""
    age_again_text = _format_age_again_email_text(aa_filters) if aa_filters else ""
