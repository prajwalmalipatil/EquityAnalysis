...
def main():
    ...
    weekly_age_again_count = len(list((input_folder / const.WEEKLY_AGE_AGAIN_FILTER_DIR_NAME).glob("*.xlsx")))
    monthly_age_again_count = len(list((input_folder / const.MONTHLY_AGE_AGAIN_FILTER_DIR_NAME).glob("*.xlsx")))
    ...
    logger.info(f"  WeeklyAgeAgain: {weekly_age_again_count} files")
    logger.info(f"  MonthlyAgeAgain: {monthly_age_again_count} files")
